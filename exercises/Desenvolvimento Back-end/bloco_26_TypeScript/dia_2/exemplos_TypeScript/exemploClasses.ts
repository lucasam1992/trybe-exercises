...
//Podemos considerar uma classe como um projeto para a criação de objetos. Uma classe Person 
//descreve os atributos de uma pessoa, por exemplo: nome, data de nascimento e cor dos olhos. Ela 
//também descreve ações que uma pessoa pode executar, como falar, comer ou andar.

// Mas a classe Person é apenas um plano para a criação de uma pessoa. Você deve criar uma
// instância de pessoa da classe Person antes que ela se torne um objeto ao qual você possa atribuir
// valores de propriedade (como definir a cor dos olhos como azul) ou chamar suas ações (como falar).
enum EyeColor {
    Black = "Pretos",
    Blue = "Azuis",
    Green = "Verdes",
...
// Também é possível dizer que uma das propriedades da nossa classe Person não é obrigatória para 
//criarmos um objeto pessoa. Podemos usar o caractere ? para marcar uma propriedade como opcional,
// o que faz com seu tipo seja um union type entre o tipo original e undefined. Se quiséssemos dizer
// que a cor dos olhos não é obrigatória nossa classe ficaria assim:
enum EyeColor {
    Black = "Pretos",
    Blue = "Azuis",
    Green = "Verdes",
