// Os Generics são modelos de código que você pode definir e reutilizar em toda a base de código.
// Eles fornecem uma forma de informar a funções, classes ou interfaces que tipo você deseja usar
// ao chamá-las, além de nos ajudar a reduzir o uso do tipo any, que não é uma boa prática em TypeScript.
...
// Declaramos uma variável de nome numberArray chamando a função getArray e passando a ela um array
// de numbers, enquanto uma variável stringArray é declarada com um array de strings. Como o tipo any
// foi usado, não há nada que impeça o código de enviar um string para o numberArray ou um number para
// o stringArray.
...
//Os generics definem uma ou mais variáveis de tipo para identificar o tipo ou tipos que serão passados
// para o componente, colocados entre colchetes angulares (< >). T é um nome comumente usado para um 
//generic, mas você pode nomeá-lo como desejar
// Após especificar a variável de tipo, você pode usá-la no lugar do tipo em parâmetros, no tipo de retorno ou 
//em qualquer outro lugar na função em que você adicionaria uma anotação de tipo.

// TypeScript impede que os itens inválidos sejam adicionados ao array.
